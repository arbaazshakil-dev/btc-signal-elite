...
def record_yesterdays_outcome(city, series_ticker):
    ...
    have one stored) so we can calibrate our bias and std going forward.

    Logs error against BOTH the corrected forecast (for backward-compatible
    display/debugging) and the raw NWS forecast (for clean bias calibration
    that doesn't compound on whatever bias was in effect at prediction time).
    """
    ...
    raw_forecast = existing.data[0]["raw_forecast"]
    error = actual_high - forecasted_high
    raw_error = actual_high - raw_forecast if raw_forecast is not None else None
    ...
    print(f"Logged outcome for {city}: forecast {forecasted_high} (raw {raw_forecast}), actual {actual_high}, error {error} (raw_error {raw_error})")

def get_calibration(city, default_bias, default_std, lookback_days=14, min_samples=5):
    """
    Compute a calibrated bias and std from recent RAW forecast errors for a
    city (actual_high - raw_forecast, before any correction was applied).

    Using raw_error rather than the already-corrected error means the bias
    we compute here is a clean, direct replacement for cfg["bias"] -- it
    doesn't compound with whatever bias was in effect on past days.

    Falls back to the hardcoded defaults if there isn't enough history yet.
    """
    ...
    raw_errors = [r["raw_error"] for r in rows.data if r["raw_error"] is not None]

    if len(raw_errors) < min_samples:
        return default_bias, default_std, len(raw_errors)

    bias = sum(raw_errors) / len(raw_errors)

    # std of residuals AFTER removing the calibrated bias -- this is the
    # genuine leftover uncertainty, not inflated by a correctable offset.
    variance = sum((e - bias) ** 2 for e in raw_errors) / len(raw_errors)
    std = math.sqrt(variance)
    std = max(std, 1.0)  # guard against a degenerate near-zero std from a lucky short streak

    return bias, std, len(raw_errors)
...
def run():
    ...
    for city, cfg in CITIES.items():
        try:
            ...
            # 3. Use calibrated bias + std if we have enough history, else fall back to defaults
            bias, std, sample_count = get_calibration(city, cfg["bias"], cfg["default_std"])
            corrected_mean = raw_forecast + bias
            print(f"{city}: using bias={bias:.2f} std={std:.2f} (from {sample_count} historical samples)")
