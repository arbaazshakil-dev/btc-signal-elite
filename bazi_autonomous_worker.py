"""BAZI V10 unattended KXBTC15M prediction, resolution, and online learning.
...
HEADERS = {"User-Agent": "BAZI-V10-Research/1.0", "Accept": "application/json"}
...
def discover_and_predict():
    ...
    if not candidates:
        print("No KXBTC15M contract in the 3–13 minute capture window.")
        return
    snap=market_snapshot()
    for seconds,market,expiry in candidates:
        side,p,decision,features,target,kp=make_prediction(market,snap,seconds)
        payload={"market_ticker":market["ticker"],"series_ticker":SERIES,
                 "expiry_at":expiry.isoformat(),"target":target,"coinbase_now":snap["cb"],
                 "kraken_now":snap["kraken"],"kalshi_yes_probability":kp,
                 "predicted_side":side,"probability":round(p,6),"decision":decision,
                 "features":features,"model_version":"v10"}
        created=sb_post("bazi_predictions",payload)
        print(("Logged" if created else "Already logged"),market["ticker"],decision,side,f"{p:.1%}")
