...
def parse_contract_seconds_remaining(end_clock_text, tz_name="America/Los_Angeles"):
    """
    Parse local HH:MM or HH:MM:SS contract end time.
    If the entered clock is already past, assume the next day.
    """
    tz = ZoneInfo(tz_name)
    now = datetime.now(tz)
    raw = end_clock_text.strip()
    parsed = None
    for fmt in ("%H:%M:%S", "%H:%M"):
        try:
            ...
            pass
    if parsed is None:
        raise ValueError("Use contract end time like 01:45:00 or 01:45.")
    expiry = datetime.combine(now.date(), parsed, tzinfo=tz)
    if expiry <= now:
        expiry += timedelta(days=1)
    return max(0.0, (expiry - now).total_seconds()), expiry
...
with st.sidebar:
    ...
    contract_end = st.text_input(
        "Contract end time (Los Angeles)",
        "01:45:00",
        help="24-hour HH:MM or HH:MM:SS. Countdown updates automatically.",
    )
...
try:
    ...
    seconds_remaining, expiry_local = parse_contract_seconds_remaining(contract_end)
    ...
    # Strict green/red contract pick:
    # target probability >=75%, no high flip risk, and at least 2 independent
    # confirmations. Validation must pass; otherwise BAZI stays LEAN/WAIT.
    ...
    if confidence < min_confidence or seconds_remaining <= 0 or (regime["choppy"] and confidence < 0.75):
        signal_level = "WAIT"
    ...
    st.write(
        f"**Contract probability:** ABOVE {target['p_above']*100:.1f}% • "
    ...
    st.write(
        f"Recent 1m realized volatility: **{target['sigma_1m']*100:.3f}%**"
    )
    st.write(
        f"Estimated short-term drift/min: **{target['mu_1m']*100:+.4f}%**"
    )
    ...
    st.caption(
        ...
        "Contract probability is a statistical estimate from target distance, countdown, volatility and market signals; it is not a guarantee of settlement or profit."
    )
