...
def _format_cell(col: str, val):
    ...
    if col.lower() in ("seconds left", "seconds_remaining"):
        try:
            total = int(float(val))
            minutes, seconds = divmod(max(0, total), 60)
            return f"{minutes:02d}:{seconds:02d}", "color:#7d8590; font-family:monospace;"
        except (ValueError, TypeError):
            return text, "color:#7d8590; font-family:monospace;"

    if col.lower() in ("created_at",):
        # Rendered as a placeholder here; render_digital_table swaps this for a
        # browser-local-time span so it matches the viewer's clock, not UTC.
        return text, "color:#7d8590; font-family:monospace;"
...
def render_digital_table(rows: list[dict], columns: list[str], labels: Optional[dict] = None,
    ...
    The table body scrolls internally (max_height_px) instead of stretching
    the page, so long histories don't force endless page scrolling. The
    header row stays pinned at the top of that scroll area. Any "created_at"
    column is converted client-side from UTC to the viewer's local timezone,
    so displayed times match the device's clock instead of showing raw UTC.
    """
    if not rows:
    ...
    for row in rows:
        ...
        for c in columns:
            raw = row.get(c)
            if c.lower() == "created_at" and raw:
                # Embed the raw UTC ISO timestamp; JS below converts it to local time.
                cells += f'<td style="color:#7d8590; font-family:monospace;"><span class="bazi-local-time" data-utc="{raw}">{raw}</span></td>'
                continue
            text, style = _format_cell(c, raw)
            cells += f'<td style="{style}">{text}</td>'
        body_html += f"<tr>{cells}</tr>"

    st.markdown(f"""
    <style>
        .digital-table-wrap {{ border-radius:12px; border:1px solid #21262d;
            max-height:{max_height_px}px; overflow-y:auto; overflow-x:auto; }}
        .digital-table {{ width:100%; border-collapse:collapse; background:#0d1117; }}
        .digital-table thead th {{ position:sticky; top:0; z-index:1;
            background:#161b22; color:#7d8590; text-align:left;
        ...
        .digital-table-wrap::-webkit-scrollbar {{ width:6px; height:6px; }}
        .digital-table-wrap::-webkit-scrollbar-thumb {{ background:#30363d; border-radius:3px; }}
    </style>
    ...
    <script>
        (function() {{
            document.querySelectorAll('.bazi-local-time').forEach(function(el) {{
...
def main():
    ...
    if cloud:
        ...
        if cloud_rows:
            ...
            render_digital_table(cloud_rows[:100], display_cols, col_labels)
    else:
    ...
    if checkpoint_cloud:
        ...
        if checkpoint_rows:
            ...
            render_digital_table(history[:100],
