...
st.set_page_config(
    page_title="BAZI BTC V8.2 Settlement Safe",
    page_icon="₿",
    layout="centered",
...
# UI
# -----------------------------
st.title("₿ BAZI BTC V8.2 — SETTLEMENT SAFE")
st.caption(
...
with st.sidebar:
    ...
    settlement_reference = st.number_input(
        ...
        format="%.2f",
        help=(
            "Enter the live BTC 'Now' / settlement-reference price shown by the contract. "
            "Use 0 if unavailable. In the final 90 seconds, BAZI will refuse a confident "
            "trade without this reference because Coinbase is not the settlement benchmark."
        ),
    )
...
try:
    ...
    # Settlement-feed alignment:
    # Kalshi crypto contracts settle from CFB RTI data, not Coinbase spot.
    # If the user supplies the live settlement-reference price shown on the market,
    # preserve the observed basis and apply it to the Coinbase expiry forecast.
    ...
    settlement_forecast_price = target["forecast_price"] + settlement_basis

    if settlement_ref_available:
        # Recalculate ABOVE/BELOW around the settlement-adjusted forecast.
    ...
    settlement_safe = settlement_ref_available or not late_contract

    # If both feeds are known, flag a material disagreement.
    basis_abs = abs(settlement_basis)
    feed_conflict = settlement_ref_available and basis_abs >= max(3.0, price * 0.00005)
    ...
    elif late_contract and not settlement_ref_available:
        forced_wait_reason = "SETTLEMENT REFERENCE REQUIRED"
    elif feed_conflict:
    ...
    if settlement_ref_available:
        ...
        if feed_conflict:
            st.error(
                f"⚠️ PRICE FEED DISAGREEMENT: Coinbase and the settlement reference "
                f"differ by ${basis_abs:.2f}. BAZI blocks a confident pick until the "
                f"feeds converge."
            )
    else:
        st.warning(
    ...
    st.caption(
        ...
        "V8.2 forecasts BTC from Coinbase but aligns late-contract target decisions to the supplied Kalshi/CF settlement reference. It blocks confident picks when settlement-feed risk is unresolved."
    )
