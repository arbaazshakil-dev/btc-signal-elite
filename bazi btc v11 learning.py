...
def _format_cell(col: str, val):
    ...
    if col.lower() in ("created_at",):
        try:
            # Parse the Supabase ISO timestamp (e.g. "2026-08-25T04:18:32.914219+00:00")
            # and render it as a clean 12-hour clock time, like "4:18 AM" — no date,
            # no seconds, no timezone clutter.
            iso = text.replace("Z", "+00:00")
            dt = datetime.fromisoformat(iso)
            return dt.strftime("%-I:%M %p"), "color:#7d8590; font-family:monospace;"
        except Exception:
            return text, "color:#7d8590; font-family:monospace;"
...
def render_digital_table(rows: list[dict], columns: list[str], labels: Optional[dict] = None,
    ...
    header row stays pinned at the top of that scroll area. Timestamps are
    formatted server-side into a clean 12-hour clock time (e.g. "4:18 AM").
    """
    if not rows:
...
def main():
    ...
    if cloud:
        ...
        if cloud_rows:
            display_cols = ["market_ticker", "decision", "predicted_side", "correct", "created_at"]
            col_labels = {"market_ticker": "Contract", "decision": "Decision",
                          "predicted_side": "Signal", "correct": "Correct", "created_at": "Time"}
