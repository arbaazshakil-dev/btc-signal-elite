...
try:
    ...
    st.title("₿ BTC Predictor Elite V5 — Precision")
    st.markdown(
        '<div style="font-size:18px;font-weight:700;margin-top:-8px;margin-bottom:8px;">BAZI</div>',
        unsafe_allow_html=True
    )
    st.caption("8-minute BTC prediction system • precision-first • iPhone-stable")
    ...
    st.write(f"**Current confidence:** {confidence*100:.1f}%")
    ...
    with st.expander("Prediction history"):
        if st.session_state.prediction_log:
            for p in reversed(st.session_state.prediction_log[-30:]):
                exit_text = "—" if p["exit_price"] is None else f"${p['exit_price']:,.2f}"
                result_text = "PENDING" if not p["settled"] else ("WIN ✅" if p["win"] else "LOSS ❌")
                st.markdown(
                    f"**{p['time'].strftime('%H:%M:%S')}** · "
                    f"{p['direction']} · {p['confidence']*100:.1f}% · "
                    f"Entry ${p['entry_price']:,.2f} · Exit {exit_text} · {result_text}"
                )
        else:
