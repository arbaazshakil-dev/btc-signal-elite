...
def checkpoint_history():
    """Read V12.1 8m/6m/4m/2m checkpoint results from Supabase."""
    try:
        url = st.secrets["SUPABASE_URL"].rstrip("/")
        key = st.secrets["SUPABASE_SERVICE_ROLE_KEY"]
    except Exception:
...
def main():
    ...
    st.divider()
    st.header("Autonomous learning V12")
    try:
        cloud = cloud_history()
    except Exception as exc:
        cloud = None
    ...
    st.caption("Shadow checkpoint tracker: independent predictions near 8, 6, 4, and 2 minutes remaining. These results do not change the main V12 model weights.")
    try:
    ...
    if checkpoint_cloud:
        ...
        cols = st.columns(4)
        for i, label in enumerate(("8m", "6m", "4m", "2m")):
        ...
        if unresolved:
            ...
            latest = [latest_by_checkpoint[x] for x in ("8m","6m","4m","2m") if x in latest_by_checkpoint]
            if latest:
        ...
        st.info("Checkpoint results will appear here after the V12.1 tracker records its first 8m/6m/4m/2m prediction.")
