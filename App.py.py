...
try:
    ...
    # Strong confirmation can happen from ~60% when independent signals agree.
    # This is intentionally more reachable than the previous all-or-nothing filter.
    strong_confirm = (
        confidence >= 0.60
        and not regime["choppy"]
        and momentum_agrees
        and (book_agrees or (similar_agrees and similar_conf >= 0.56))
    )

    # Extremely strong model confidence can still qualify with one supporting signal.
    if confidence >= 0.66 and not regime["choppy"] and (momentum_agrees or book_agrees):
        strong_confirm = True

    if confidence < min_confidence or (regime["choppy"] and confidence < 0.60):
        signal_level = "WAIT"
